...
- Reads `.txt`/`.csv`/`.dat` (plain columns, including "wide" multi-column
  files where each extra column is its own spectrum sharing column 1's
  Raman-shift axis), `.dpt` (OPUS/Bruker), `.spc` (Galactic/GRAMS binary,
  also used by Jobin-Yvon/Horiba systems), and `.wdf` (Renishaw WiRE
  binary).
